...
public:
  ...
  Memory(const Config &configs, vector<Controller<T> *> ctrls)
    ...
    ramulator_active_cycles.name("ramulator_active_cycles")
        .desc("The total number of cycles that the DRAM part is active "
              "(serving R/W)")
        .precision(0);
    ...
    record_read_requests.init(configs.get_core_num())
        ...
        .desc("record read requests for this core when it reaches request "
              "limit or to the end");

    record_write_requests.init(configs.get_core_num())
        .name("record_write_requests")
        .desc("record write requests for this core when it reaches request "
              "limit or to the end");
#endif
  }
  ...
  unsigned get_channel_num() { return 8; }
  ...
  void dump_mapping_scheme() {
    ...
    out << "Mapping Scheme: " << endl;
    for (MapScheme::iterator mapit = mapping_scheme.begin();
         mapit != mapping_scheme.end(); mapit++) {
      int level = mapit->first;
      for (MapSchemeEntry::iterator entit = mapit->second.begin();
           entit != mapit->second.end(); entit++) {
        out << T::level_str[level] << "[" << entit->first << "] := ";
        out << "PhysicalAddress[" << *(entit->second.begin()) << "]";
        entit->second.erase(entit->second.begin());
        for (MapSrcVector::iterator it = entit->second.begin();
             it != entit->second.end(); it++)
          out << " xor PhysicalAddress[" << *it << "]";
        out << endl;
      }
    }
  }
  ...
  void apply_mapping(long addr, std::vector<int> &addr_vec) {
    ...
    addr_bits[int(T::Level::Row)] =
        min((int)sizeof(int) * 8,
            max(addr_total_bits, calc_log2(sz[int(T::Level::Row)])));
...
private:
  ...
  bool get_bit_at(long addr, int bit) { return (((addr >> bit) & 1) == 1); }
